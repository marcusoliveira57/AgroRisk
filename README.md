...
| `simulador.py`       | Simulador interativo de comparação entre duas culturas com projeção Auto-ARIMA e benchmark Selic  |
| `validar_risco.py`   | Auditoria matemática passo a passo do cálculo de volatilidade (Desvio Padrão Amostral)            |
| `testar_modelo.py`   | Backtesting do modelo preditivo com cálculo de erro MAE mês a mês                                 |
...
- Retorno esperado projetado (Auto-ARIMA)
- Nível de risco (Baixo / Moderado / Alto)
- Lucro projetado sobre o investimento simulado
- Comparação com o rendimento da Selic no mesmo período
...
Testa a acurácia do modelo ARIMA nos últimos N meses da série histórica e calcula o erro médio absoluto (MAE).
...
│   ├── testar_modelo.py          # Backtesting do modelo Auto-ARIMA
...
2. **Risco** = Desvio Padrão Amostral das margens históricas (Correção de Bessel, N-1)
3. **Retorno projetado** = média das previsões do modelo **Auto-ARIMA/SARIMA** para o ciclo da safra
4. **Benchmark** = rendimento equivalente da **Selic** no mesmo período de capital travado
